<div class="mb-4">
    @if (!empty($image_url))
        <div class="app-btn-add-image has-image " style="background-image: url({{ $image_url }})">
            ...
            <div class="btn-actions p-2">
                <a href="#" class="btn btn-primary app-btn-icon app-btn-center-inline" @click="$store.basic.imagePick($el, event)"
                 data-target="input_{{ $field }}">
                    @component('icons.btn.camera')  @endcomponent
                </a>
                <a href="{{ $delete_url }}" class="btn btn-primary app-btn-icon app-btn-center-inline" >
                    @component('icons.btn.delete')  @endcomponent
                </a>
            </div>
        </div>
    @else
        <a href="#" class="app-btn-add-image text-decoration-none" @click="$store.basic.imagePick($el, event)"
            data-target="input_{{ $field }}">
            <div>
    ...
    <div class="py-4 d-none">
        <input type="file" name="{{ $field }}" id="input_{{ $field }}"
            x-on:change="$store.basic.changeImagePick($el)">
    </div>
